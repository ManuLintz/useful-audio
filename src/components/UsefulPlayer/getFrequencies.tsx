/**
 * Inspired by the spectrogram plugin in wavesurfer.js
 * https://github.com/katspaugh/wavesurfer.js/blob/4a92a2d3d85e4e96d679af0029eb6dd215a0b492/src/plugins/spectrogram.ts
...
export function getFrequencies(buffer: AudioBuffer): Uint8Array[] {
  ...
  const WIDTH = 1000 // Find out what it means

  //   this.frequencyMax = this.frequencyMax || buffer.sampleRate / 2

  // Probably unused in FFT
  //   this.buffer = buffer
  ...
  const fft = new FFT(fftSamples, sampleRate, 'hann', 0.16)
  const frequencies = []
  ...
  while (currentOffset + fftSamples < channelData.length) {
    ...
    // Get Spectrum of Size
    const spectrum = fft.calculateSpectrum(segment)
    const array = new Uint8Array(fftSamples / 2) // I don't know why we do /2 here
